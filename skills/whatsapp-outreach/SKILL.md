...
version: 3.0.0
description: Use when 需要通过WhatsApp触达客户或批量发送WhatsApp消息时。路由：WhatsApp 触达必须用此技能（内置72h铁律、联系记录），禁止直接调 whatsapp_send。
always: false
triggers:
  - WhatsApp
  - 发WhatsApp
...
# whatsapp-outreach — WhatsApp触达 v3.0
...
> v3.0 更新：内置 `whatsapp_send` 工具替代 wacli CLI，无需外部依赖。基于 Baileys WebSocket 协议，无需 Chromium/Puppeteer。QR 码扫码绑定一次后持久化 session。
...
```
客户有WhatsApp号码？
  ├─ 是 → 检查 WhatsApp 认证状态（whatsapp_auth_status 工具）
  │        ├─ 已认证 → 生成WhatsApp风格消息 → 确认 → 调用 whatsapp_send 发送
  │        └─ 未认证 → 提示用户在 设置 > 渠道 > WhatsApp 个人版 中扫码绑定
...
5. **发送**：调用内置工具 `whatsapp_send({to: "号码", message: "消息"})`
6. **文件发送**（可选）：`whatsapp_send({to: "号码", message: "说明", file: "/path/file.pdf", caption: "文件说明"})`
...
## 工具参考（holo-desktop 内置）

### whatsapp_send — 发送消息

通过个人 WhatsApp 账号发送文本或文件。

```
参数：
  to (必需):    收件人号码，国际格式（如 966501234567 或 +966501234567）
  message (必需): 消息文本（50-150词，半正式语气）
  file (可选):   附件路径（PDF、图片、视频）
  caption (可选): 文件说明文字
```

### whatsapp_auth_status — 检查认证

```
返回：
  authenticated: true/false
  state: "disconnected" | "connecting" | "qr_ready" | "authenticated" | "error"
  accountName: WhatsApp 显示名称
  accountNumber: 绑定的手机号
```

### 首次绑定

1. 打开 holo-desktop 设置 > 渠道 > WhatsApp 个人版
2. 点击"扫描二维码绑定"
3. 用手机 WhatsApp → 设置 → 关联设备 → 扫码
4. 绑定成功后 session 自动持久化，下次启动无需重新扫码
...
## 历史参考：wacli CLI（v3.0 后已废弃）

v3.0 起改用 holo-desktop 内置的 `whatsapp_send` 工具。wacli 内容折叠保留：

<details>
<summary>wacli CLI 命令（点击展开）</summary>

wacli auth / wacli doctor / wacli send text|file / wacli sync|chats|messages

注意事项：sync/send 不能同时运行、JID 格式 `号码@s.whatsapp.net`、QR码有效期约20秒、LOCK文件残留问题。
...
_版本: 3.0.0_
_更新: 2026-05-25_
_依赖: holo-desktop 内置 whatsapp_send 工具（Baileys WebSocket 协议）_
_实战验证: 2026-04-06 沙特客户 8/8 批量发送成功（wacli CLI 版本）_
