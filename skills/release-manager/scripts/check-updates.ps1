...
# 路径解析：scripts/release-manager/scripts -> release-manager -> skills -> workspace root
# 打包目标是整个 workspace（包含 skills/ memory/ customer-data/ 等子目录）
$scriptDir = Split-Path -Parent $MyInvocation.MyCommand.Path
$skillRoot = Split-Path -Parent (Split-Path -Parent $scriptDir)  # -> release-manager
$skillsParent = Split-Path -Parent $skillRoot                              # -> skills (or workspace)

if ($WorkspacePath -eq "") {
    # 优先从 .release 所在目录推断
    if (Test-Path (Join-Path $skillsParent ".release")) {
        $WorkspacePath = $skillsParent
    } elseif (Test-Path (Join-Path $skillRoot ".release")) {
        $WorkspacePath = $skillRoot
    } else {
        # fallback：skills 本身就是 workspace
        $WorkspacePath = $skillsParent
    }
}
...
if (Test-Path $SkillsPath) {
    Get-ChildItem $SkillsPath -Directory -Depth 0 -ErrorAction SilentlyContinue | Where-Object { $_.Name -notmatch '^(IsReadOnly|IsFixedSize|IsSynchronized|Count)$' } | ForEach-Object {
