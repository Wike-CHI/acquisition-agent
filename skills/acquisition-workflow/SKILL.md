...
> **Skill Graph：** 领域 → [[_index-meta|系统元技能领域]] | 上游 ← [[_index-acquisition|核心流程领域]] | 下游 → [[acquisition-coordinator|任务编排器]]（引用本流程）


> **⚠️ 重要：读取配置优先**
> 执行本技能前，必须先读取以下配置文件获取最新业务数据：
> - `../../config/company-profile.json` — 公司信息（名称/地址/联系方式/竞品）
> - `../../config/infrastructure.json` — NAS 路径（IP/共享目录/本地路径）
>
> **禁止**在后续流程中使用硬编码的公司信息，应使用配置文件中读取的值。
> 本文件中的硬编码内容将在后续迭代中逐步迁移至配置文件。
