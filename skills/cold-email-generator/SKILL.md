...
version: 2.2.0
description: "Use when 用户需要写开发信、cold email、外联邮件、起草销售邮件时。路由：此技能只生成邮件内容不发送，生成后必须交给 email-sender 技能发送（email-sender 内置ICP校验、邮箱铁律、联系记录），禁止直接调 send_email_smtp 发送开发信。"
...
| 你要做的 | 执行 |
|---------|------|
| 给新客户写开发信 | ① `check_existing_customer` 黑名单检查 ② 检查 NAS 档案 ③ 用初稿模板生成 ④ 调用 sdr-humanizer 润色 ⑤ 评分 ≥9.0 |
| 写跟进邮件 | 查 `references/email-sequence.md` 对应步次的要素和 CTA 策略 |
| 客户已回复 | 切换到 `inquiry-response`，不再用本技能 |
| 需要 NAS 资料 | 查 `references/nas-paths.md` |
| 黑名单命中 | 立即停止，告知业务员"该客户已是成交客户，禁止发开发信" |

---

## 核心原则

0. **黑名单前置检查（铁律）** — 发开发信前必须调用 `check_existing_customer` 检查是否为成交客户。命中则禁止发送！
...
- **默认**：在对话中直接展示开发信内容，方便业务员立即查看和复制
- **保存为 Word**：如果业务员要求保存，用 `word-docx` 技能生成 .docx 文件，使用开发信模板（含发件人信息、日期、收件人、签名）
- **不要用 Excel**：开发信是纯文本内容，用 Word 或对话展示，不要生成 .xlsx
...
```
check_existing_customer (黑名单检查 — 铁律！)
       ↓
  命中？→ 是 → 立即停止！告知"该客户已是成交客户，禁止发开发信"
       ↓ 否
