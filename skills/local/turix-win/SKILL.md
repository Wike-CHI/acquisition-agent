...
```powershell
powershell -ExecutionPolicy Bypass -File "C:\Users\Administrator\.openclaw\workspace\skills\local\turix-win\scripts\run_turix.ps1" "<TASK>"
```
- If the user explicitly asks for background execution:
```powershell
Start-Process powershell -ArgumentList "-ExecutionPolicy","Bypass","-File","C:\Users\Administrator\.openclaw\workspace\skills\local\turix-win\scripts\run_turix.ps1","<TASK>"
```
- If the user explicitly asks to continue an interrupted run:
```powershell
powershell -ExecutionPolicy Bypass -File "C:\Users\Administrator\.openclaw\workspace\skills\local\turix-win\scripts\run_turix.ps1" --resume <AGENT_ID>
```
